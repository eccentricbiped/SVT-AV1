 ...
 * used in the second pass of two pass encoding
 ******************************************************/
static int adaptive_qindex_calc_tpl_la(
    PictureControlSet         *picture_control_set_ptr,
    RATE_CONTROL              *rc,
    int                        qindex) {
...
static void sb_qp_derivation_tpl_la(
    ...
    if (picture_control_set_ptr->parent_pcs_ptr->frm_hdr.delta_q_params.delta_q_present) {
        ...
        for (sb_addr = 0; sb_addr < sequence_control_set_ptr->sb_tot_cnt; ++sb_addr) {
            ...
            for (int mby_offset = 0; mby_offset < (sequence_control_set_ptr->seq_header.sb_size == BLOCK_128X128 ? 8 : 4); mby_offset++) {
                for (int mbx_offset = 0; mbx_offset < (sequence_control_set_ptr->seq_header.sb_size == BLOCK_128X128 ? 8 : 4); mbx_offset++) {
                    ...
                    //printf("kelvin ---> mbx=%d mby=%d picture_width_in_mb=%d\n", mbx, mby, picture_width_in_mb);
            ...
            offset = av1_get_deltaq_offset(sequence_control_set_ptr->static_config.encoder_bit_depth, pcs_ptr->frm_hdr.quantization_params.base_q_idx, beta);
            ...
            if (sequence_control_set_ptr->static_config.rate_control_mode == 0) {
                ...
                if (sequence_control_set_ptr->static_config.enable_qp_scaling_flag && picture_control_set_ptr->parent_pcs_ptr->qp_on_the_fly == EB_FALSE) {
                    ...
                    if (!sequence_control_set_ptr->use_output_stat_file && picture_control_set_ptr->parent_pcs_ptr->frames_in_sw >= QPS_SW_THRESH) {
                        ...
                        // Content adaptive qp assignment
                        if(/*!picture_control_set_ptr->parent_pcs_ptr->sc_content_detected &&*/ picture_control_set_ptr->parent_pcs_ptr->r0 != -1)
                            new_qindex = adaptive_qindex_calc_tpl_la(
                                picture_control_set_ptr,
                                &rc,
                                qindex);
                        else
